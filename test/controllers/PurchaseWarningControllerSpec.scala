...
class PurchaseWarningControllerSpec extends SpecBase {
  ...
  "PurchaseWarning Controller" - {
    ...
    "must redirect to DescribeItemsOnInvoiceController on submit in NormalMode" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual routes.DescribeItemsOnInvoiceController.onPageLoad(NormalMode).url
      }
    }

    "must redirect to JourneyRecoveryController on submit in CheckMode" in {
